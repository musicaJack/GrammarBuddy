...
void HomeView::init(lv_obj_t* parent)
{
    ...
    lv_obj_set_style_text_font(_status, &lv_font_montserrat_28, 0);

    _sub_status = lv_label_create(_root);
    lv_label_set_text(_sub_status, "");
    lv_obj_set_style_text_color(_sub_status, lv_color_hex(0xF2F4F8), 0);
    lv_obj_set_style_text_align(_sub_status, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_font(_sub_status, &lv_font_montserrat_28, 0);
    lv_obj_set_width(_sub_status, LV_PCT(90));
    lv_label_set_long_mode(_sub_status, LV_LABEL_LONG_WRAP);

    _hint = lv_label_create(_root);
    lv_label_set_text(_hint, "BtnA retry | A+B home");
    lv_obj_set_style_text_color(_hint, lv_color_hex(0xAAB2C0), 0);
    lv_obj_set_style_text_font(_hint, &lv_font_montserrat_18, 0);
    lv_obj_set_style_text_align(_hint, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_width(_hint, LV_PCT(90));
    lv_label_set_long_mode(_hint, LV_LABEL_LONG_WRAP);
}
